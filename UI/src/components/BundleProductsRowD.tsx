...
export default function BundleProductsRowD({
  rowTitle,
  rowStartLocation,
  products,
  deleteFromBundle,
  ...
  return (
    <div className="flex w-fit justify-center gap-10 p-5">
      {products.map((product, index) => {
        if (index === 3 && product.id === '') {
          return (
            <div key={index} className="min-h-68 w-68">
              <div
          ...
          return (
            <div key={index} className="min-h-68 w-68">
              ...
              <p className="text-center text-xs text-black">{product.title}</p>
            </div>
          )
        }
      })}
